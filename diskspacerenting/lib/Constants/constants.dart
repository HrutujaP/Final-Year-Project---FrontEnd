import 'package:flutter/material.dart';

// const Color kPrimaryColor = Color(0xFF160B33);
const Color kPrimaryColor1 = Color.fromARGB(255, 154, 72, 255);
// const Color kPrimaryColor2 = Color(0xff150a34);
// const Color kSecondaryColor = Colors.white;
const Color kSecondaryColor1 = Colors.white;
const Color kItemColor = Color.fromARGB(255, 120, 216, 235);
const Color kItemColor2 = Color.fromARGB(255, 234, 183, 235);
// const Color kSecondaryColor2 = Color(0xFF84DBAE);
// const Color kSecondaryColor3 = Color(0xFFC8F48E);
const Color kRedColor = Colors.red;
const Color kPurpleColor = Color.fromARGB(255, 141, 8, 165);

const Color kColor1 = Color.fromARGB(255, 204, 193, 232);
const Color kColor2 = Color.fromARGB(255, 60, 16, 89);
const Color kColor3 = Color.fromARGB(255, 138, 76, 188);

const Color kPrimaryColor = Color(0xFF007AFF);
const Color kPrimaryColor2 = Color(0xFF87CEEB);
const Color kPrimaryColor3 = Color(0xFFADD8E6);
const Color kPrimaryColor4 = Color(0xFF87CEEB);
const Color kPrimaryColor5 = Color(0xFF89CFF0);
const Color kPrimaryColor6 = Color(0xFFB0E0E6);
const Color kSecondaryColor = Color(0xFFFFFFFF);
const Color kSecondaryColor2 = Color(0xFF8E8E93);
const Color kSecondaryColor3 = Color(0xFFC0C0C0);
const Color kContrastColor = Color(0xFF000000);




// const Color kColor1 = Color(0xFF745A6E);
// const Color kColor2 = Color(0xFF867183);
// const Color kColor3 = Color(0xFF523F53);
